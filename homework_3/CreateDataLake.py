...
watch_next_dataset_filtered = watch_next_dataset_raw.drop_duplicates().where('url != "https://www.ted.com/session/new?context=ted.www%2Fwatch-later"')

print(f"Number Watch_Next items (RAW): {watch_next_dataset_raw.count()}")
print(f"Number Watch_Next items: {watch_next_dataset_filtered.count()}")

# ADD WATCH NEXT DETAILS
watch_next_dataset = watch_next_dataset_filtered.join(tedx_dataset_agg.drop("tags", "details", "url"), watch_next_dataset_filtered.watch_next_idx == tedx_dataset_agg._id, "left")

watch_next_dataset.printSchema()

# ADD WATCH_NEXT TO AGGREGATE MODEL

watch_next_dataset_agg = watch_next_dataset.groupBy(col("idx").alias("idx_ref_watch_next")).agg(collect_list(struct("_id", "url", "title", "posted", "main_speaker")).alias("watch_next"))
